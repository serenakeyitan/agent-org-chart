...
Each chart lives at `charts/<id>/chart.json` and is a single importable package.
...
| `version` | Chart package version (e.g. `1.0.0`). |
| `tags` | String labels for discovery. |
| `principles` | Operating principles every spawned agent should follow. |
| `roles` | Array of role objects (orchestrator, specialists, optional peers). |
...
| `persona` | Full system/profile text. Platform-agnostic; say “the user,” not a specific person. |
...
Copy `charts/company-os/`, change `id` / content, validate against the JSON schema, and add a row to [`charts/INDEX.md`](./charts/INDEX.md).
