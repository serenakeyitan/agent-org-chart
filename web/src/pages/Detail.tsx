...
import { armyOwner, plateLabel } from '../lib/catalog'
...
export default function Detail({ chart, plateNumber, selectedRoleId, onSelectRole }: DetailProps) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const next = i === -1 ? (step === 1 ? 0 : chart.roles.length - 1) : (i + step + chart.roles.length) % chart.roles.length
    ...
    <div className="page page--detail">
      ...
      <div className="study__body">
        <figure className="artboard">
          <OrgChart key={chart.id} roles={chart.roles} selectedRoleId={selectedRoleId} onSelectRole={onSelectRole} />
          <figcaption className="artboard__caption">
            <span>
              <em>Plate {plateNumber !== null ? String(plateNumber).padStart(2, '0') : ''}</em> — {chart.title}
            </span>
